...
import random, torch
from probe_common import clean_exit, make_llm, ragged_step
...
try:
    ...
    mr = llm.model_runner
    with torch.inference_mode():
        graphed = mr.run_model(ids, pos, True).clone()  # replay; fills persistent buffers
        ...
        set_context(True, v["cu_q"][:sl + 1], v["cu_k"][:sl + 1], tp,
                    mr.config.max_model_len, v["slot_mapping"][:tp], None,
                    v["block_tables"][:sl])
        eager_padded = mr.model.compute_logits(
            mr.model(v["input_ids"][:tp], v["positions"][:tp])
        )
