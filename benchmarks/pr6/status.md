...
| C2 | bucketed varlen graphs + run_model routing + miss counter; prepare_ragged block_tables condition | wip `523c19e` + P10 fix in tree (pre-touch moved post-default-restore, production-shaped inputs). P10/P11 RESOLVED: step-1 1075→293 ms, counters == novarlen control (p11_host1.txt). 39 tests green; bench miss attributed to host load, not branch — re-check band on quiet host. Remaining for C2 close: byte-gate continuity vs base.json (one-time axis-2 token-gate at branch-change entry), C2-stage bench_latency | targets: per-bucket bitwise; byte-gate continuity vs base.json; 39 tests; bench band; C2-stage bench_latency (predict: interactive TTFT 27→7–10 ms, long TTFT 39→8–14, max_ITL only ~15–20 — stall residual is C3's job) |
