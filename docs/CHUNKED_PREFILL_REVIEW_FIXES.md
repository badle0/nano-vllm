...
> **Follow-up on `fork-main`:** the historical review below predates the
> incremental allocator and invariant backend now implemented. Current behavior
> is documented in [Invariant numerics and speculative-decoding optimization](NUMERICAL_AND_SPECULATIVE_OPTIMIZATIONS.md). The retained evidence in this file
> remains useful provenance for the original failures and focused repairs.
